...
class FastCLIPAttention2(nn.Module):
    ...
    def __init__(self, hidden_size: int, out_dim: int, num_attention_heads: int, out_seq_len: Optional[int] = None,
                 norm_qk: bool = False):
        super().__init__()
    ...
    def forward(self, query_states: torch.Tensor, kv_states: torch.Tensor) -> torch.Tensor:
        ...
        q_states = q_states.view(bsz, tgt_len, self.num_heads, self.head_dim).transpose(1,
                                                                                        2)  # (bsz, num_heads, tgt_len, head_dim)
        k_states = k_states.view(bsz, src_len, self.num_heads, self.head_dim).transpose(1,
                                                                                        2)  # (bsz, num_heads, src_len, head_dim)
        v_states = v_states.view(bsz, src_len, self.num_heads, self.head_dim).transpose(1,
                                                                                        2)  # (bsz, num_heads, src_len, head_dim)
...
class CLIPEmbeddingLayer(nn.Module):
    def __init__(self, hidden_size: int, num_channels: int, image_size: int, patch_size: int,
                 patch_dropout: float = 0.0, good_dropout: bool = False, dpn: bool = False,
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if self.sine_positional_embeddings:
            position_embeddings = sinusoidal_position_embedding(W // self.patch_size, H // self.patch_size,
                                                                self.hidden_size, pixel_values.dtype,
                                                                pixel_values.device)
        else:
...
class MHAPoolingHead(nn.Module):
    def __init__(self, hidden_size: int, num_attention_heads: int, activation_cls, out_dim: int, alt_style: bool,
                 norm_qk: bool):
        super().__init__()
...
class CLIPLikeModel(VisionModel):
    def __init__(
        ...
        stochastic_depth_space = torch.linspace(0, stochastic_depth,
                                                num_layers) if stochastic_depth is not None else None

        self.embedding_layer = CLIPEmbeddingLayer(embedding_dim, num_channels, image_size, patch_size, patch_dropout,
                                                  good_dropout, dpn, sine_positional_embeddings)
        ...
        if use_gap_head:
            ...
            self.pooling_head = MHAPoolingHead(embedding_dim, num_attention_heads, activation_cls, out_dim, use_mha_alt,
                                               norm_qk=norm_qk)
...
class MaskedAutoEncoderViT(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.sine_positional_embeddings:
            position_embeddings = sinusoidal_position_embedding(W // self.patch_size, H // self.patch_size,
                                                                self.embedding_dim, pixel_values.dtype, device)
            decoder_position_embeddings = sinusoidal_position_embedding(W // self.patch_size, H // self.patch_size,
                                                                        self.decoder_embedding_dim, pixel_values.dtype,
                                                                        device)
        else:
            position_embeddings = self.positional_embeddings(self.position_ids)
...
class PosEmbedding(nn.Module):
    ...
    def forward(self, x, width: int, height: int):
        if self.use_sine:
            position_embeddings = sinusoidal_position_embedding(width // self.patch_size, height // self.patch_size,
                                                                self.d_model, x.dtype, x.device)
        else:
...
class ViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        q_states = qkv_states[0].view(bsz, src_len, self.num_heads, embed_dim // self.num_heads).transpose(1,
                                                                                                           2)  # (bsz, num_heads, src_len, embed_dim // num_heads)
        k_states = qkv_states[1].view(bsz, src_len, self.num_heads, embed_dim // self.num_heads).transpose(1,
                                                                                                           2)  # (bsz, num_heads, src_len, embed_dim // num_heads)
        v_states = qkv_states[2].view(bsz, src_len, self.num_heads, embed_dim // self.num_heads).transpose(1,
                                                                                                           2)  # (bsz, num_heads, src_len, embed_dim // num_heads)
...
class ViT(VisionModel):
    def __init__(self,
        ...
        self.pos_embedding = PosEmbedding(d_model, (image_size // patch_size) ** 2, use_sine=use_sine,
